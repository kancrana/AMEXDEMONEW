...
{
    [RoutePrefix("api/ProductCatalog")]
    public class ProductCatalogController : ApiController
    {
        ...
        public HttpResponseMessage FindById(int id)
        {
            ...
            if (item != null)
            {
                return Request.CreateResponse(HttpStatusCode.OK, _productRepository.FindBy(id));
            }
        ...
        public HttpResponseMessage ListItemsWithName(string name, [FromUri] int pageSize = 10, [FromUri] int pageIndex = 0)
        {
        ...
        public async Task<HttpResponseMessage> UpdateProductAsync([FromBody] Product product)
        {
            ...
            return Request.CreateResponse(HttpStatusCode.Created, product.Id);
        }

        [HttpGet]
        [Route("items/type/{productTypeId:int}/brand/{productBrandId?}")]
        public HttpResponseMessage ListItemsWithTypeAndBrand(int productTypeId, [FromUri] int? productBrandId = null, [FromUri] int pageSize = 10, [FromUri] int pageIndex = 0)
        {
        ...
        public async Task<HttpResponseMessage> GetProductBrandsAsync()
        {
            ...
            {
                return Request.CreateResponse(HttpStatusCode.OK, productBrands);
            }
